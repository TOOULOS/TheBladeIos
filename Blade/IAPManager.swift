...
let IS_PURCHASED = "purchase_date"
let IS_EXPIRE = "expires_date"
...
class IAPManager : NSObject {
    ...
    func getSubscriptionData(_ identifier: String) -> Dictionary<String, Any>? {
        return UserDefaults.standard.object(forKey: identifier) as? Dictionary<String, Any>
    }
    
    func expirationDateFor(_ identifier : String) -> String? {
        return UserDefaults.standard.object(forKey: identifier) as? String
    }
    
    func purchaseDateFor(_ identifier : String) -> String? {
        return UserDefaults.standard.object(forKey: identifier) as? String
    }
        ...
        for receipt in receipts_array {
            ...
            if let date = formatter.date(from: receipt["expires_date"] as! String) {
                if date > Date() {
                    ...
                    UserDefaults.standard.set(formatter.string(from:date), forKey: IS_EXPIRE)
                    UserDefaults.standard.set(receipt, forKey: LAST_RECEIPT)
                } else {
                    UserDefaults.standard.set(nil, forKey: IS_EXPIRE)
                    UserDefaults.standard.set(nil, forKey: LAST_RECEIPT)
                }
            }
            ...
            if let date = formatter.date(from: receipt["purchase_date"] as! String) {
                formatter.dateFormat = "dd-MMM-yyyy"
                UserDefaults.standard.set(formatter.string(from:date), forKey: IS_PURCHASED)
            }
        }
